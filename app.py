...
# Recommendations dictionary (kept as you had it...)
treatment_recommendations = { ... }
...
def predict_disease():
    ...
    # Fix 1: Secure the filename and make it unique to avoid collisions
    temp_dir = 'temp_images'
    os.makedirs(temp_dir, exist_ok=True)
    ...
    try:
        ...
        # Prediction
        predictions = model.predict(img_array)
        predicted_index = np.argmax(predictions[0]) # index 0 for batch
        
        predicted_disease = disease_classes[predicted_index]
        confidence_score = float(np.max(predictions[0]))

        treatment = treatment_recommendations.get(predicted_disease, "Consult local experts.")
        ...
        # Fix 2: Ensure cleanup happens even if an error occurs earlier
        if os.path.exists(img_path):
            os.remove(img_path)
...
if not os.path.exists(MODEL_FILE):
    # This ensures the app doesn't crash cryptically if the model is missing
...
# Dictionary for treatment recommendations (omitted for brevity, keep yours here!)
treatment_recommendations = {
...
def predict_disease():
    ...
    try:
        ...
        treatment = treatment_recommendations.get(predicted_disease, "Consult local agricultural extension.")
        ...
        # Cleanup: Always delete the file after processing
        if os.path.exists(img_path):
            os.remove(img_path)
...
if __name__ == '__main__':
    print("Starting AgroLens Disease Detection API...")
    # Change debug=False in production for better security
    disease_app.run(host='0.0.0.0', port=5000, debug=True)